...
data "aws_ami" "ami" {
  ...
  filter {
    name   = "tag:version"
    values = [var.api_version_to_deploy]
  }
}
...
resource "aws_key_pair" "api-keypair" {
  key_name   = "KP_API_${var.color}_${var.environment}"
...
resource "aws_launch_configuration" "api_launch_configuration" {
  ...
  name                 = "api-${var.environment}-${var.color}-${var.api_version_to_deploy}"
...
resource "aws_autoscaling_group" "api_asg" {
  ...
  tags = [
    {
      key                 = "Name"
      value               = "api-${var.color}-${var.api_version_to_deploy}-autoscaled"
...
resource "aws_cloudwatch_metric_alarm" "cpu_high_alarm" {
  alarm_name          = "awsec2-api-cpu-high-${var.color}-${var.environment}"
...
resource "aws_cloudwatch_metric_alarm" "cpu_low_alarm" {
  alarm_name          = "awsec2-api-cpu-low-${var.color}-${var.environment}"
