...
module "api" {
  ...
  name                        = "api"
  authorized_cidrs            = var.api_ingress_cidr
  ipv6_authorized_cidrs       = []
  redis_node_count            = 2
...
module "deploy" {
  ...
  environment           = var.name
  api_version_to_deploy = var.version_to_deploy
  ...
  target_group_arn      = module.api.api_target_group_arn
  api_role              = module.api.api_role_id
  cluster_size          = var.api_cluster_size
  cluster_size_max      = var.api_cluster_size
}
